...
//
// The conv<Numeric|DateTime>ToString family shares one defect: bound to a
// text parameter that Firebird describes as UTF8, the driver mapped the slot
// to SQL_C_WCHAR and picked the wide converter, which wrote UTF-16 code units
// into the Firebird buffer (duckdb/odbc-scanner#161).  The pre-existing suite
// only covered SLONG → INTEGER/SMALLINT round-trips, so none of the affected
// converters were tested.
//
...
//
// The column declares CHARACTER SET UTF8 explicitly so the tests take the
// wide-mapped path on any test database.  They skip on the FB6 snapshot,
// which aborts parameterized statements server-side (SKIP_ON_FIREBIRD6).
...
protected:
    void SetUp() override {
        ...
        ExecDirect("CREATE TABLE ODBC_NUM_VAR ("
                   ...
                   "VAL VARCHAR(40) CHARACTER SET UTF8)");
        Commit();
        ReallocStmt();
    }
...
// fix in this PR.  That's a separate pre-existing bug and should
